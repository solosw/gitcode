...
public interface OrigizationMapper extends BaseMapper<Origization> {
    ...
    @Select("select id,name,description,create_time,update_time,creator_id,av from origization where creator_id=#{userId} or CAST(member_ids AS VARCHAR) like CONCAT('%', CAST(#{userId} AS VARCHAR), '%')")
